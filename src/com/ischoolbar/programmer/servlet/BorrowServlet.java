...
public class BorrowServlet extends HttpServlet {
	...
	private void returnBorrow(HttpServletRequest request,
		...
		if(borrow == null){
			ret.put("type", "error");
			ret.put("msg", "请选择要购票的信息!");
			StringUtil.writrToPage(response, JSONObject.toJSONString(ret));
			return;
		}
		if(borrow.getNumber() < realReturnNumber){
			ret.put("type", "error");
			ret.put("msg", "退票数量大于购票数量，请检查头脑!");
			StringUtil.writrToPage(response, JSONObject.toJSONString(ret));
			return;
		}
		...
		if(book.getStatus() == 2){
			//若公交状态是全部售出
			book.setStatus(1);//设置成状态为可售
		}
		if(!bookDao.update(book)){
			ret.put("type", "error");
			ret.put("msg", "公交信息更新失败!");
			StringUtil.writrToPage(response, JSONObject.toJSONString(ret));
			return;
		}
